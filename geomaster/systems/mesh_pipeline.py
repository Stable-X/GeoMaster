...
def prepare_data(source_path, resolution=None):
    ...
    for camera in cameras:
        ...
        if os.path.exists(normal_path):
            ...
            _normal = torch.tensor(_normal_array).cuda().float() / 255 * 2 - 1
            _normal *= -1
    ...
    return imgs, depths, normals, grayimgs, masks, w2cs, projs, poses, len(imgs)
...
@click.option('--normal_weight', default=0.2, type=float, help='Normal weight')
@click.option('--mask_weight', default=0., type=float, help='Mask weight')
@click.option('--normal_l1_weight', default=0.1, type=float, help='Normal L1 weight')
@click.option('--depth_weight', default=0.8, type=float, help='Depth weight')
...
@click.option('--start_edge_len', default=0.04, type=float, help='#smallest allowed reference edge length')
@click.option('--end_edge_len', default=0.001, type=float, help='#largest allowed reference edge length')
@click.option('--laplacian_weight', default=0.02, type=float, help='laplacian weight for laplacian smoothing/regularization')
def main(source_path, model_path, output_path, num_points, num_sample, h_patch_size, ncc_thresh, lr,
    ...
    # Load sparse
    imgs, depths, gt_normals, grayimgs, masks, w2cs, projs, poses, num = prepare_data(source_path, resolution)
    ...
    for iteration in pbar:
        ...
        update_pbar_description_depth(pbar, depth_loss, normal_loss, normal_l1_loss)
...
def update_pbar_description_depth(pbar, depth_loss, normal_loss, normal_l1_loss):
