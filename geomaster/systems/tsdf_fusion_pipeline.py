...
def tsdf_fusion(dataset, output_path, depth_dir="depths", vox_size=0.02):
    ...
    # ======================================================================================================== #
    cam_infos = dataset.all_cameras

    fov_x = cam_infos[0].FoVx
    fov_y = cam_infos[0].FoVy
    image_width = cam_infos[0].image_width
    image_height = cam_infos[0].image_height
    principal_point_ndc = cam_infos[0].principal_point_ndc

    fx = image_width / (2 * math.tan(fov_x / 2))
    fy = image_height / (2 * math.tan(fov_y / 2))

    cam_intr = np.array([
        [fx, 0, principal_point_ndc[0] * image_width],
        [0, fy, principal_point_ndc[1] * image_height],
        [0, 0, 1]
    ])
    # ======================================================================================================== #
    ...
    for cam_info in cam_infos:
        ...
        cam_pose[:3, 3] = -np.dot(cam_pose[:3, :3], cam_info.T.reshape(3, 1).squeeze())
    ...
    tsdf_vol = TSDFVolume(vol_bnds, voxel_size=vox_size)
    ...
    meshwrite(os.path.join(output_path, "mesh.ply"), verts, faces, norms, colors)
    meshwrite_trimesh(os.path.join(output_path, "trimesh.ply"), verts, faces)

    # Get point cloud from voxel volume and save to disk (can be viewed with Meshlab)
    print("Saving point cloud to pc.ply...")
    point_cloud = tsdf_vol.get_point_cloud()
    pcwrite(os.path.join(output_path, "pc.ply"), point_cloud)
    # ======================================================================================================== #
