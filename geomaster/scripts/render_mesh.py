...
#        rotated_vertices = np.dot(vertices, rotation_matrix.T)
        ...
        for i in range(num_all_views):
            ...
            # Normalize and mask background
            normal_norm = torch.norm(view_normal[..., :3], dim=-1, keepdim=True)
            background_mask = normal_norm > 1.1
            view_normal[..., :3] = view_normal[..., :3] / normal_norm
            view_normal[background_mask.squeeze(-1)] = 0
            view_normal = -1*view_normal
            
            save_image(view_normal, output_dir, i + 1)
